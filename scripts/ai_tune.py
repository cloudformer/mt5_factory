...
"""AI 调参一轮(Claude 订阅版, 无 API key): 在装了 Claude Code 且已登录的机器上跑。
    ...
    python3 scripts/ai_tune.py 177 --count 12 --base http://localhost:8000 --cross

流程: 拉 /strategies/ai/prompt.txt(成绩单+纪律已拼好) → 喂 `claude -p`(用你登录的订阅额度)
      → 提取返回 JSON → POST 回 /strategies/ai/submit(校验/生成子代/自动回测)
...
def main():
    ...
    ap.add_argument("--cross", action="store_true", help="回测带交叉品种")
    a = ap.parse_args()
