...
- **bridge**（端口 9090）：MT5↔HTTP 转换，供 Linux app 下载数据、心跳、远程下发账户
- **runner**：从 app 拉取 DEMO/ACTIVE 策略，收盘 bar 决策，本地直接下单（必带 SL/TP）
...
# 3. worker.env 只需填 APP_URL (MT5 账户可留空)
notepad worker.env
# 4. 启动 (或注销重登, 计划任务自动拉起)
.\start_bridge.bat
.\start_runner.bat
...
2. worker.env 里填 `MT5_LOGIN/PASSWORD/SERVER`
3. 从 Linux 侧远程下发：`curl -X POST <app>/hosts/1/connect -d '{"login":..,"password":"..","server":".."}'`

验证：

```bash
curl http://<windows_ip>:9090/health     # bridge + MT5 状态
```
...
VALUES ('win-worker-1', '192.168.x.x', 9090, '{download,backtest,live}', 'DEMO');
```

## Bridge API（端口 9090 固定）
...
- 每 60s 从 app 刷新策略清单（`RUN_STATUS` 决定跑 DEMO 还是 ACTIVE）
