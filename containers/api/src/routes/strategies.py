...
2) **格间分离 vs 格内离散**: separation = 各格全量 PF 的离散程度(建议取 log 后按笔数加权
   求标准差); dispersion = 每格逐年 PF 的离散程度(同法算完按笔数加权汇总);
...
- 倍率按证据强度给, 不是按 PF 高低给: 最硬 = 1.0, 中等 = 0.6~0.8,
  弱 = 【不入选】(别用低倍率把证据不足的格硬塞进来)。
...
- half_rho ≤0 或 dispersion 是 separation 数倍 → 最高只能 medium;
