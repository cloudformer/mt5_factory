...
async def market_overview(request: Request):
    ...
    syms = [r["symbol"] for r in await pool.fetch(
        "SELECT symbol FROM symbols WHERE download ORDER BY symbol")]
    # 版本订阅数按状态分列(2026-08-10 Frank 定): 总数 / 实盘(DEMO+LIVE=worker在跑) /
    # 空闲(CANDIDATE) / 归档 — 删版本前看谁还在用, 实盘列尤其是红线
    subs: dict = {}
    for r in await pool.fetch(
            "SELECT (metadata->'regime'->>'version')::int AS vid, status, count(*)::int AS n"
            "  FROM strategies WHERE metadata->'regime'->>'version' IS NOT NULL"
            " GROUP BY 1, 2"):
        d = subs.setdefault(r["vid"], {"total": 0, "live": 0, "idle": 0, "archived": 0})
        d["total"] += r["n"]
        if r["status"] in ("DEMO", "LIVE"):
            d["live"] += r["n"]
    ...
    for v in await pool.fetch("SELECT id, params FROM regime_versions ORDER BY id"):
        ...
        versions.append({
            ...
            "subs": subs.get(v["id"]) or {"total": 0, "live": 0, "idle": 0, "archived": 0},
...
async def _version_save(pool, params: dict, owner: int = 1,
    ...
    if vid is None:
        row = await pool.fetchrow(
            "INSERT INTO regime_versions (params, owner_id) VALUES ($1, $2)"
            " ON CONFLICT (params) DO NOTHING RETURNING id", params, owner)
        if row is not None:
...
async def set_config(key: str, req: ConfigUpdate, request: Request):
    ...
    if key == "regime_view":  # Regime页默认视图: 色带跨度(0=全部) + 象限窗(0=全历史, 负=N年以前)
        if (not isinstance(req.value, dict)
                or req.value.get("band_years") not in (0, 1, 3, 5, 10)
                or req.value.get("quad_years") not in (0, 1, 2, 3, 5, 10, -1, -2, -3, -5)):
            raise HTTPException(status_code=400,
                                detail="regime_view 须为 {band_years: 0/1/3/5/10,"
                                       " quad_years: 0/±1/2/3/5/10(负=N年以前, 仅负1/2/3/5)}")
