...
> 1. **引擎单元测试**（触发条件：改撮合/OOS/对账代码之前）：数据层设计已认可不动
>    （回测主键 UPSERT 覆盖、调参=建新实例不改旧）；对账是实盘路径的事后回归防线，
>    单元测试补"事前网"（已知K线输入→应得成交输出的用例集），两者互补
> 2. **[v1.3] /backtest/top 排序分页下推 SQL**：现在全量拉进 Python 排序再切页；
>    改为翻页时每页现查（ORDER BY + LIMIT/OFFSET，排名百分位用窗口函数）。
>    JSONB 热字段（净点/PF）是否提拔真列建索引，届时一并评估（现阶段不动）
> 3. **point 漂移哨兵**（待细化讨论）：trades.net_points 是入库时按当时 point 算好的存量，
>    point 变则新老不同尺 — 对账时抽笔用原始价格×当前 point 复算自检，不符亮警告；
>    真漂移再按原始价格批量重算治愈
> 4. **兜底默认值不落代码**（2026-07-19 定）：配置唯一源=数据库（schema 种子），
>    web/api 不得把同一串默认值写死当兜底 — api 不可用就显示空+报错，不用假数据顶
> 5. **[v2.3 多券商启用前] symbols 主键加 broker**：现同名品种跨券商挤一行共用 point；
>    单券商期间不动。届时连带：品种登记/下载/回测按 (symbol, broker) 定位
>
> 下载编排进度在内存（上面欠账第一条）2026-07-19 复核：K线落库幂等不丢数据，
> 中断重点一次即按缺口续补 — 单副本+人工触发下无实际风险，维持记账即可
>
